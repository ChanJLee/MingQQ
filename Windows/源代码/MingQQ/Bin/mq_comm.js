...
XIAOKU.Encryption = function() {
    ...
    function getEncryption(password, salt, vcode, isMd5) {
        ...
        s2 = md5(h1 + salt),
        rsaH1 = XIAOKU.RSA.rsa_encrypt(h1),
        rsaH1Len = (rsaH1.length / 2).toString(16),
		//修改处：TEA改成window.TEA
        hexVcode = window.TEA.strToBytes(vcode.toUpperCase()),
        vcodeLen = "000" + vcode.length.toString(16);
        while (rsaH1Len.length < 4) {
            rsaH1Len = "0" + rsaH1Len
        }
		//修改处：TEA改成window.TEA
        window.TEA.initkey(s2);
		//修改处：TEA改成window.TEA
        var saltPwd = window.TEA.enAsBase64(rsaH1Len + rsaH1 + window.TEA.strToBytes(salt) + vcodeLen + hexVcode);
		//修改处：TEA改成window.TEA
        window.TEA.initkey("");
    ...
    function getRSAEncryption(password, vcode, isMd5) {
        ...
        var str3 = XIAOKU.RSA.rsa_encrypt(str2);
        return str3
    }
    return {
...
//调用
//XIAOKU.Encryption.getEncryption('wqy13186561890',"\x00\x00\x00\x00\x3c\xcb\x48\x45", 'zuwk');
...
function calcPwdHash(ptuin, password, vcode) {
    eval('ptuin="' + ptuin + '"');
    return XIAOKU.Encryption.getEncryption(password, ptuin, vcode);
}